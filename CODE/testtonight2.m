% Part one: Accumulating data
% Using CloseS to store occuring realtime prices
...
ticker1 = app.Stock1EditField.Value;
ticker2 = app.Stock2EditField.Value;
ticker3 = app.Stock3EditField.Value;
ticker4 = app.Stock4EditField.Value;
ticker5 = app.Stock5EditField.Value;
ticker6 = app.Stock6EditField.Value;
ticker7 = app.Stock7EditField.Value;
ticker8 = app.Stock8EditField.Value;
ticker9 = app.Stock9EditField.Value;
ticker0 = app.Stock10EditField.Value;

% Below are the codes for backtesting callback function
...
% Assign the call back codes to timers
...
% Accumulating 48 bar data for each stock
pause(240)

% Stop backtesting timers
stop(timer_bt1);
stop(timer_bt2);
stop(timer_bt3);
stop(timer_bt4);
...
% Convert price series into series because the MACD function can only take
% table as inputs
...
% Part Two Backtesting
% Put converted tables into backtesting
...
% Initial positions = last backtesting signal
...
% establish a eventable to store events
cells = {'', ''};
...
% establish initial position for each stock
...
% establish a vector to store past unrealized returns
unrealized_series = [];

% Call back function codes for live trading 
% for each stock
code1 = ["i1 = i1+1;"...
...
code_ret = ["Portf = IBMatlab('action', 'portfolio');"...
    "unrealized = sum([Portf.unrealizedPnL]);"...
    "display(['Current Unrealized Return: ', num2str(unrealized)]);"...
    "unrealized_series = [unrealized_series unrealized];"...
    "if length(unrealized_series)>100;"...
    "unrealized_series = unrealized_series(2:end);"...
    "end;"...
    "plot(unrealized_series);"...
    "fprintf('\n');"];

% assign call back functions to each timer
...
% pause for one second and then display current unrealized return
start(timer_ret);
